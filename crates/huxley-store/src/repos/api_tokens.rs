...
pub trait ApiTokensRepository: Send + Sync {
    ...
    async fn list(&self, conn: &mut PgConnection, page: PageQuery) -> HuxleyStoreResult<Page<ApiTokenModel>>;
    async fn list_by_user_id(&self, conn: &mut PgConnection, user_id: Uuid, page: PageQuery) -> HuxleyStoreResult<Page<ApiTokenModel>>;
...
impl ApiTokensRepository for PgApiTokensRepository {
            ...
            PageSort::Asc => {
                sqlx::query_as!(
                    ...
                    r#"
                        SELECT api_token_id, user_id, name, prefix, scopes, last_used_at, expires_at, revoked_at, created_at, updated_at
                        FROM api_tokens
                        WHERE ($2::bigint IS NULL OR api_token_id >= $2)
            ...
            PageSort::Asc => {
                sqlx::query_as!(
                    ...
                    r#"
                        SELECT api_token_id, user_id, name, prefix, scopes, last_used_at, expires_at, revoked_at, created_at, updated_at
                        FROM api_tokens
                        WHERE ($2::bigint IS NULL OR api_token_id >= $2) AND (user_id = $3)
                        ORDER BY id ASC
                        LIMIT $1 + 1
                    "#,
                    resolved_limit,
    ...
    async fn update(&self, conn: &mut PgConnection, id: Uuid, input: UpdateApiToken) -> HuxleyStoreResult<ApiTokenModel> {
        ...
        let (set_scopes, scope) = input.scopes.into_parts();
